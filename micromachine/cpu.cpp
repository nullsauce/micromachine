...
cpu::cpu()
	...
	, _mem(_interrupter,{
		...
		std::make_pair(NVIC_ISER, std::ref(_nvic.iser_reg())),
		std::make_pair(NVIC_ICER, std::ref(_nvic.icer_reg())),
		std::make_pair(NVIC_ISPR, std::ref(_nvic.ispr_reg())),
		std::make_pair(NVIC_ICPR, std::ref(_nvic.icpr_reg())),
