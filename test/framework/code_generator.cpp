...
helpers::code_generator::code_generator()
	: _write_address(0)
	, _base_address(0)
	, _mem(nullptr) {}

void helpers::code_generator::set_mem(memory* mem) {
	_mem = mem;
}

void helpers::code_generator::set_write_address(uint32_t address) {
	_write_address = address;
}

uint32_t helpers::code_generator::write_address() const {
	return _write_address;
}

void helpers::code_generator::emit_ins16(const char* encoding, ...) {
	va_list valist;
...
void helpers::code_generator::emit_ins32(const char* encoding1,
...
uint16_t helpers::code_generator::assemble_instruction(const char* encoding,
																			 va_list valist) {
	uint16_t instr = 0;
	size_t i = 0;
	char last = '\0';
