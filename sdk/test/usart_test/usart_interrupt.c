 ...
 */
typedef void (*xfer_completed_cb)(volatile struct usart_device* dev);
...
void receive_data(volatile struct usart_device* usart_dev) {
...
void send_data(volatile struct usart_device* usart_dev) {
...
static
void usart_wait_tx_complete(volatile struct usart_device* dev) {
	while(false == dev->tx_completed){};
}

static
void usart_wait_rx_complete(volatile struct usart_device* dev) {
...
void main() {
	...
	usart_wait_tx_complete(&usart_device);

	/*
	 * Async read of data are generated by the vm
	 */

	if(usart_device.port->ISR & USART_ISR_RXNE) { // check if there's anything to read

		// We except the vm to send _exactly_ 16 bytes
		const size_t expected_data_size = 15;
		uint8_t receive_buffer[32] = {0};
		usart_receive_async(&usart_device, receive_buffer, expected_data_size);
		usart_wait_rx_complete(&usart_device);
		printf("%s\n", receive_buffer);
	} else {
		printf("There was no uart data to read!\n");
	}
