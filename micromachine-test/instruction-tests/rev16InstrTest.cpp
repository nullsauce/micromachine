...
MICROMACHINE_TEST_F(rev16, RevR0toR7, CpuTestFixture) {
	emitInstruction16("1011101001mmmddd", registers::R0, registers::R7);
	setReg(registers::R0, 0x12345678);
	StepAndExpectThatInstruction16IsExecutedAndThat(reg(registers::R7).equals(0x34127856));
}

MICROMACHINE_TEST_F(rev16, RevR7toR0, CpuTestFixture) {
	emitInstruction16("1011101001mmmddd", registers::R7, registers::R0);
	setReg(registers::R7, 0x12345678);
	StepAndExpectThatInstruction16IsExecutedAndThat(reg(registers::R0).equals(0x34127856));
}
