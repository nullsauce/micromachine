...
public:
	static void check(const mcu& predictedState, const mcu& actualState) {
		...
		for(size_t i = 0; i < numRegions; i++) {
			...
			for(size_t offset = 0; offset < predictedRegion.size(); offset++) {
				...
				if(predictedByte != actualByte) {
					EXPECT_PRED_FORMAT5(assertMemoryByteEquality, predictedByte, actualByte, offset, actualRegion,
										predictedRegion);
					break;
				}
			}
